...
def create_map(routes, shapes, stops, stop_routes, ridership_df, active_routes=None):
    ...
    stop_ridership = ridership_df[ridership_df['Route'].isin(active_routes)].groupby(['Stop', 'Name'])['Ridership'].sum().reset_index()
    ...
    print(f"Unique Stop IDs for Active Routes: {active_stop_ids}")
    print("Active Stops after Filtering and before Merge:")
    print(active_stops[['stop_id', 'stop_lat', 'stop_lon', 'stop_name']].head())
    ...
    # Check if any ridership values are NaN
    if active_stops['Ridership'].isna().sum() > 0:
        print(f"Warning: Found {active_stops['Ridership'].isna().sum()} stops with NaN ridership values.")
